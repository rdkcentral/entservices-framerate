...
namespace WPEFramework
{
    namespace Plugin
    {
        ...
        void FrameRateImplementation::OnDeviceSettingsActivated()
        {
            ...
            if (vd != nullptr) {
                ...
                if (rc == Core::ERROR_NONE) {
                    ...
                    LOGINFO("VideoDevice handle acquired: %d", _videoDeviceHandle);
                } else {
        ...
        Core::hresult FrameRateImplementation::GetDisplayFrameRate(string& framerate, bool& success)
        {
            ...
            string fr;
            Core::hresult result = vd->GetCurrentDisplayFrameRate(_videoDeviceHandle, fr);
            vd->Release();
            if (result == Core::ERROR_NONE) {
                framerate = fr;
                success = true;
        ...
        Core::hresult FrameRateImplementation::GetFrmMode(int &autoFRMMode, bool& success)
        {
            ...
            int32_t frfmode = 0;
            Core::hresult result = vd->GetFRFMode(_videoDeviceHandle, frfmode);
            vd->Release();
            if (result == Core::ERROR_NONE) {
        ...
        Core::hresult FrameRateImplementation::SetDisplayFrameRate(const string& framerate, bool& success)
        {
            ...
            Core::hresult result = vd->SetDisplayFrameRate(_videoDeviceHandle, sFramerate);
            vd->Release();
        ...
        Core::hresult FrameRateImplementation::SetFrmMode(int frmmode, bool& success)
        {
            ...
            Core::hresult result = vd->SetFRFMode(_videoDeviceHandle, static_cast<int32_t>(frmmode));
            vd->Release();
            success = (result == Core::ERROR_NONE);
            if (!success) {
